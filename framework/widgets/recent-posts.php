...
class sprouts_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'sprouts_rp', // Base ID
			__('Sprouts Recent Posts with Thumbnails', 'sprouts'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'sprouts' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'sprouts' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'sprouts' );
		}
		?>
		<p>
		...
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'sprouts' ); ?></label>
...
add_action( 'widgets_init', 'register_sprouts_widget' );  
function register_sprouts_widget() {  
    register_widget( 'sprouts_Recent_Posts' );  
}  
